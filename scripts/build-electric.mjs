// scripts/build-electric.mjs
// 전기(Krones 전기·제어) chunks + 임베딩 매칭 + GPT 강사노하우 요약 생성.
// 라벨러/제병기/팩커 데이터는 절대 수정하지 않고 electric-* 만 추가.
//
// 사전조건: data/manuals-raw/_pdf/*.pdf (LibreOffice 변환본, pptx-to-webp.mjs가 생성),
//          public/slides/electric-*/ webp 존재, data/manuals-raw/electric/*.txt 녹취록
// 실행: OPENAI_API_KEY=... node scripts/build-electric.mjs
...
const TXT_DIR = "data/manuals-raw/electric";   // 전기 녹취록만 (다른 설비와 분리)
const CACHE = "data/manuals-raw/_summaries-cache-electric.json";
...
const MANUALS = [
  { pdf: "전기일반_01_설비관리 3.pdf",          fileName: "전기일반_01_설비관리 3.pptx",          fid: "electric-01", content_type: "설비관리" },
  { pdf: "전기일반_02 설비이론 3.pdf",          fileName: "전기일반_02 설비이론 3.pptx",          fid: "electric-02", content_type: "설비이론" },
  { pdf: "전기일반_03_설비셋팅 3.pdf",          fileName: "전기일반_03_설비셋팅 3.pptx",          fid: "electric-03", content_type: "설비세팅" },
  { pdf: "전기일반_04_설비 트러블 TOP 13.pdf",  fileName: "전기일반_04_설비 트러블 TOP 13.pptx",  fid: "electric-04", content_type: "트러블슈팅" },
];
// 녹취록: electric 폴더의 모든 .txt (전기 강의)
  ...
  for (const fn of TRANSCRIPTS) {
    ...
    for (let i = 0; i < norm.length;) {
      ...
      if (text) {
        gi++; ci++;
        out.push({
          chunk_id: `electric_chunk_${String(gi).padStart(3, "0")}`, source_type: "transcript",
          file_name: fn, file_id: "electric-transcripts", chunk_index: ci,
...
async function summarize(slide, tops) {
  const hasCtx = tops.length > 0;
  const ctx = hasCtx ? tops.map((t) => `[${t.file_name}]\n${t.text}`).join("\n---\n") : "(관련 녹취 없음 — 슬라이드 내용 기반으로 정리)";
  const sys = `당신은 Krones 음료라인 전기·제어(전장/PLC/서보/인버터/센서/통신) 설비 전문가입니다. 아래 강의 녹취록과 슬라이드에서 현재 슬라이드 주제와 관련된 '현장 노하우'를 추출해 요약하세요.
규칙:
- 반드시 "핵심 노하우:" 로 시작하고, 그 아래 핵심 노하우를 "*   " 불릿으로 2~4개 정리
- 녹취에 실제로 있는 실무 팁/수치/주의사항 우선. 녹취가 없거나 빈약하면 슬라이드 내용 기반으로 핵심만 간결히 (일반론 나열 금지)
- 한국어, 300자 이내`;
...
async function main() {
  ...
  const transVecs = trans.length ? await embedAll(trans.map((t) => t.text)) : [];

  // 3) 슬라이드별 top-K 녹취 (전기 녹취 풀 안에서만)
  ...
  // 5) chunks.json 추가 (다른 설비 보존, electric-* 만 교체)
  const chunks = JSON.parse(readFileSync("data/chunks.json", "utf8"));
  const keptChunks = chunks.filter((c) => !(String(c.file_id).startsWith("electric")));
